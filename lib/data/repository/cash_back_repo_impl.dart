...
class CashBackRepoImpl implements CashBackRepository {
  ...
  @override
  Future<Either<Failure, CashBackProductTypesModel>> getCashBackProductTypes() async {
    try {
      final response = await _apiService.get(ApiRoutes.distributorCashBackProductTypes);
      final model = CashBackProductTypesModel.fromJson(response);

      if (model.success == true) {
        return Right(model);
      } else {
        return Left(ServerFailure(model.message ?? "Failed to fetch cashback product types."));
      }
  ...
  @override
  Future<Either<Failure, CashBackModel>> getCashBackList(String productTypeId) async {
    try {
      ...
      if (model.success == true) {
        ...
        return Left(ServerFailure(model.message ?? "Failed to fetch cashback list."));
      }
    } on DioException catch (e) {
      return Left(ServerFailure(e.message ?? 'A network error occurred'));
    } catch (e) {
