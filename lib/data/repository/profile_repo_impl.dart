...
class ProfileRepositoryImpl implements ProfileRepository {
    ...
    try {
      ...
      if (model.code == 200 || model.code == 201) {
        if (model.success == true) {
          ...
          return Left(
            ServerFailure(model.message ?? 'Failed to update profile'),
          );
        }
      } else {
        return Left(
  ...
  @override
  Future<Either<Failure, GetProfileResponseModel>> verifyUpdateProfileOtp(
    String otp,
  ) async {
    try {
  ...
  @override
  Future<Either<Failure, UpdateProfileResponseModel>>
  resendUpdateProfileOtp() async {
    try {
      final response = await _apiService.post(ApiRoutes.updateProfileResendOtp);
  ...
  @override
  Future<Either<Failure, UpdateStatusSendOtpResponseModel>> updateStatusSendOtp(
    int isActive,
  ) async {
    try {
      ...
      if (model.code == 200 || model.code == 201) {
        if (model.success == true) {
          ...
          return Left(
            ServerFailure(
              model.message ?? 'Failed to send OTP for status update',
            ),
          );
        }
      } else {
  ...
  @override
  Future<Either<Failure, GetProfileResponseModel>> verifyUpdateStatusOtp(
    String otp,
  ) async {
    try {
      final formData = FormData.fromMap({"otp": otp});
